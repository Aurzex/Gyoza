 ...
 *
 * 关键设计：显示用的 <img> 一律不带 crossorigin（避免图床不支持 CORS 时
 * 图片直接加载失败）；取色用独立的探测 Image（crossOrigin）——只有服务器
 * 支持 CORS 才能读到像素，成功则取色渐变，失败则保持默认统一色。
 */
export function initImageTint(cardSelector: string) {
  const apply = (img: HTMLImageElement) => {
    const card = img.closest(cardSelector)
    if (!(card instanceof HTMLElement) || card.dataset.tinted === 'true') return
